...
export default function LeaderboardScreen() {
    ...
    const [currentUser, setCurrentUser] = useState<LeaderboardUser>(defaultLeaderboardUsers[0]);
    const [bestStreak, setBestStreak] = useState<number>(0);

    // โหลดข้อมูลและซิงค์อันดับผู้ใช้
    const loadData = async () => {
        try {
            const savedUsers = await AsyncStorage.getItem('stretchmanLeaderboardUsers');
            let parsedUsers = savedUsers ? JSON.parse(savedUsers) : [...defaultLeaderboardUsers];
            ...
            const index = parsedUsers.findIndex((u: LeaderboardUser) => u.id === updatedCurrent.id);
            if (index === -1) {
                parsedUsers.push(updatedCurrent);
            } else {
                parsedUsers[index] = { ...parsedUsers[index], ...updatedCurrent };
            }

            await AsyncStorage.setItem('stretchmanLeaderboardUsers', JSON.stringify(parsedUsers));
    ...
    return (
        <View style={styles.container}>
            <ScrollView contentContainerStyle={styles.leaderboardPage} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.leaderboardHeader}>
                    ...
                    <TouchableOpacity style={styles.historyButton} onPress={() => router.push('/streak-history' as any)}>
                ...
                <View style={styles.streakCard}>
                    ...
                    <View style={styles.streakDays}>
                        {daysOfWeek.map((dayName, index) => {
                            ...
                            const isCompleted = currentUser.streak > 0 && index <= mondayIndex && index >= mondayIndex - currentUser.streak + 1;
                            return (
                ...
                <View style={styles.leaderboardSection}>
                        ...
                        {[topThree[1], topThree[0], topThree[2]].map((user, idx) => {
                            ...
                            return (
                                <View key={user.id} style={styles.podiumPlayer}>
                                    ...
                                    <View style={[
                                        styles.rankBadge,
                                        isFirst ? styles.firstBadge : isThird ? styles.thirdBadge : styles.secondBadge
                                    ]}>
                            ...
                            return (
                                <View key={user.id} style={[styles.rankingItem, isMe && styles.currentUserItem]}>
                                    ...
                                    <View style={styles.miniAvatar}>
                                        <Text style={{ fontSize: 15 }}>{user.avatar}</Text>
                                    </View>
                    ...
                    <View style={styles.myRankingBanner}>
                        ...
                        <View style={styles.myRankingUser}>
                            <View style={styles.miniAvatar}>
                                <Text style={{ fontSize: 15 }}>{currentUser.avatar}</Text>
                            </View>
                            <View>
